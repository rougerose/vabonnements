...
function vabonnements_init($element) {
...
function toggleInput($input, $offre, chargement) {
	var classeInputParent = '.fsa__choix',
		classeSelected = 'is-selected';
		
	if (chargement === true) {
		$input.parents(classeInputParent).toggleClass(classeSelected);
	} else {
		if ($input.attr('name') == 'abonnements_offre') {
			$offre.removeClass(classeSelected);
		}
		$input.parents(classeInputParent).toggleClass(classeSelected).siblings().removeClass(classeSelected);
	}
}
...
function toggleAffichageOffres(classeOffre, $offres, chargement) {
	...
	if (chargement === false) {
		...
		$offres.eq(deselectedIndex).hide().addClass(classeAbsolute);
